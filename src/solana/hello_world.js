...
import {createKeypairFromJSON, getPayer, getRpcUrl} from './utils';
 ...
 */
const PROGRAM_SO_PATH = './config/solana_buybak_fsop.so';

/**
 * Path to the keypair of the deployed program.
 * This file is created when running `solana program deploy dist/program/solana_buybak_fsop.so`
 */
export const PROGRAM_KEYPAIR_PATH = require('./config/solana_buybak_fsop-keypair.json');
...
const ClientPairInstruction_ClientOne: number                   = 0;
const ClientPairInstruction_ClientTwo: number                   = 1;
const ClientPairInstruction_ClientThree: number                 = 2;
const ClientPairInstruction_InitializeAccount: number           = 3;
const ClientPairInstruction_FindRetailer: number                = 4;
const ClientPairInstruction_InitUserPortfolio: number           = 5;
const ClientPairInstruction_MintToUserPortfolio: number         = 6;
const ClientPairInstruction_ReturnFromUserPortfolio: number     = 7;
const ClientPairInstruction_UpdateStockPrices: number           = 8;
...
export async function establishConnection(): Promise<void> {
  ...
  // TMD console.log('Connection to cluster established:', rpcUrl, version);
}
...
export async function checkProgram(stock_seed: string): Promise<void> {
      ...
      try {
        const programKeypair = await createKeypairFromJSON(PROGRAM_KEYPAIR_PATH);
      ...
      if (greetedAccount === null) {
            ...
            console.log( 'MESSAGING_SIZE: ' + MESSAGING_SIZE + ', lamports: ' + lamports);
      ...
      {
        // TMD console.log('checkProgram: Pubkey already exists: ', greetedPubkey.toBase58());
      }
}
  ...
  console.log('Saying hello to', greetedPubkey.toBase58());
    ...
    if ( inst === 0) {
        payload = new ClientPairPayload({
            variant: ClientPairInstruction_ClientOne,
            price: price,
            quantity: quantity,
            retailer: retailer,
            stock: stock
        ...
        payload = new ClientPairPayload({
            variant: ClientPairInstruction_ClientTwo,
            price: price,
            quantity: quantity,
            retailer: retailer,
            stock: stock
        ...
        payload = new ClientPairPayload({
            variant: ClientPairInstruction_ClientThree,
            price: price,
            quantity: quantity,
            retailer: retailer,
            stock: stock
        ...
        payload = new ClientPairPayload({
            variant: ClientPairInstruction_InitializeAccount,
            price: price,
            quantity: quantity,
            retailer: retailer,
            stock: stock
        ...
        payload = new ClientPairPayload({
            variant: ClientPairInstruction_FindRetailer,
            price: price,
            quantity: quantity,
            retailer: retailer,
            stock: stock
        ...
        payload = new ClientPairPayload({
            variant: ClientPairInstruction_UpdateStockPrices,
            price: price,
            quantity: quantity,
            retailer: retailer,
            stock: stock
...
export async function SolanaExample(): Promise<Void> {

    let keypair = web3.Keypair.generate();

    // console.log( "SolanaExample: keypair: " + keypair.toBase58());
    ...
    console.log('Calling Buffer Allocate');
    let data = Buffer.alloc(allocateStruct.layout.span);
    console.log('Calling Object.assign');
...
export async function InitUserPortfolio(
    ...
    if ( inst === 6) {
        let user_payload = new InitUserPayload({
            variant: ClientPairInstruction_InitUserPortfolio,
            username: username,
            fullname: fullname,
            email: email,
            phone: phone,
        ...
        let user_payload = new UpdateUserPayload({
            variant: ClientPairInstruction_MintToUserPortfolio,
            username: username,
            fsop: fsop,
        ...
        let user_payload = new UpdateUserPayload({
            variant: ClientPairInstruction_ReturnFromUserPortfolio,
            username: username,
            fsop: fsop,
            average_price: average_price,
            stock: stock,
...
export async function getStockQuote(stock_seed: string): Promise<void> {
    ...
    const bfr = Buffer.from(accountInfo.data);
    // console.log({bfr});
    ...
    const packetLen = bfr.readInt32LE(0);
    console.log('packetLen: ' + packetLen);

    let packet = bfr.slice(8,8+packetLen);
    // console.log( {packet});
...
export async function getBTreeMap(stock_seed: string): Promise<void> {
    ...
    for( x = 0; x < btreeCount; ++x)
    {
        ...
        stockList.push({price: price, quantity: quantity, retailer: retailer, stock: stock});
...
export const json_rsp = require("./config/moomoo.json");

export async function fetchLiveQuotes() {

        console.log( new Date());

        // TODO CORS issue with moomoo. Not Now let json_rsp = await fetch_watchlist();
...
export async function getBuybakStatisticsMap(stock_seed: string): Promise<void> {
    ...
    // TMD console.log( 'getBuybakStatisticsMap from account: ' + greetedPubkey.toBase58());
    ...
    for( x = 0; x < btreeCount; ++x)
    {
        ...
        statsList.push({value: value, transactions: transactions, stock: stock});
